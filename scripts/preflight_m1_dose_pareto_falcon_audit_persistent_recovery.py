...
def main() -> None:
    ...
    if output.exists() or output.parent != root / "preflight":
        raise FileExistsError(f"audit-persistent preflight output invalid/existing: {output}")
    ...
    active = run(
        "squeue", "-h", "-n",
        "m1-v4-eval-falcon,m1-v4-eval-falcon-recovery,m1-v4-eval-falcon-a6000-recovery,m1-v4-eval-falcon-a6000-exclusive,m1-v4-eval-falcon-clean-uuid,m1-v4-eval-falcon-audit-persistent,m1-v4-falcon-summary",
        "-o", "%A|%T|%j|%r",
    )
    ...
    write_json(
        output,
        {
            **family,
            ...
            "audit_persistent_contract_sha256": FALCON_EVALUATION_AUDIT_PERSISTENT_SHA256,
            ...
            "job_shape": "one_non_array_job_sequential_steps_126_210_252",
            "selection_rule": "lexicographically_smallest_clean_gpu_uuid",
            "failure_audit_required": True,
