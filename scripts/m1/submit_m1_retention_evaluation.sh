...
prepare_id=$(sbatch --parsable slurm/m1/prepare_m1_retention_evaluation.slurm)
preflight_id=$(sbatch --parsable --dependency="afterok:${prepare_id}" \
  slurm/m1/preflight_m1_retention_evaluation.slurm)
...
evaluation_id=$(sbatch --parsable --dependency="afterok:${preflight_id}" --array="0-21%3" \
  ...
  slurm/m1/eval_m1_retention_checkpoints.slurm)
summary_id=$(sbatch --parsable --dependency="afterok:${evaluation_id}" \
  slurm/m1/summarize_m1_retention_evaluation.slurm)
audit_id=$(sbatch --parsable --dependency="afterany:${evaluation_id}" \
  slurm/m1/audit_m1_retention_evaluation.slurm)
