#!/usr/bin/env python3
"""Single lowercase-label OSCAR audit recovery over preserved V3 bytes."""
...
PRIOR_ROOT = Path("/vol/tmp2/yesildau/vngrs_m2_oscar_audit_recovery_v1")
OUTPUT_ROOT = Path("/vol/tmp2/yesildau/vngrs_m2_oscar_audit_recovery_retry_v1")
MATERIALIZATION_SHA256 = "bb413e9acafd0d891ea9c4461abc77acea1c737489560650db56dacf781dbd10"
V3_FAILURE_SHA256 = "a341e4787e38720f27beeaf5815331ef0163084cb2974d91799ee5ffe426c52f"
PRIOR_LABEL_INVENTORY_SHA256 = "178991f311336a4acead1bb00fe8043d60a18c590e809b11629ca64f4920ee2b"
PRIOR_FAILURE_SHA256 = "6ce9a5dfc302498e4713ed03293962ca53f40f97daef28ffeea49d8fbc2e813b"
...
def _validate_preflight(repo: Path, expected_commit: str) -> None:
    if OUTPUT_ROOT.exists():
        raise ValueError("fresh lowercase OSCAR recovery root is not absent")
    ...
    evidence = {
        ...
        SOURCE_V3_ROOT / "control/d0_failure.json": V3_FAILURE_SHA256,
        PRIOR_ROOT / "reports/corpus_label_inventory.json": PRIOR_LABEL_INVENTORY_SHA256,
        PRIOR_ROOT / "control/d0_failure.json": PRIOR_FAILURE_SHA256,
    }
    for path, expected in evidence.items():
        if not path.is_file() or path.is_symlink() or _sha256(path) != expected:
            raise ValueError(f"preserved recovery evidence drift: {path}")
    ...
    if usage.free < MIN_AVAILABLE_BYTES or stat.f_favail < MIN_AVAILABLE_INODES:
        raise ValueError("lowercase OSCAR recovery scratch capacity gate failed")
...
def main() -> int:
    ...
    try:
        ...
        result = run_oscar_audit_recovery(
            SOURCE_V3_ROOT, OUTPUT_ROOT, objects,
        ...
        if not (OUTPUT_ROOT / "control/d0_failure.json").exists() and not OUTPUT_ROOT.exists():
            write_d0_failure(OUTPUT_ROOT, phase="lowercase_oscar_audit_recovery_preflight", error=exc)
        _comment(f"LOWERCASE_OSCAR_AUDIT_FAILED:{type(exc).__name__}:{exc}")
        raise
