...
def main() -> None:
    parser = argparse.ArgumentParser(description="Freeze and verify the WP3 Stage A launch manifest.")
    ...
    if (train_rows, validation_rows) != (2800, 400):
        raise ValueError(f"Unexpected WP3 row counts: train={train_rows} validation={validation_rows}")
    ...
    if steps != 252:
        raise ValueError(f"Unexpected WP3 optimizer-step count: {steps}")
    ...
    if not str(resolved_output).startswith(("/vol/tmp/yesildau/", "/vol/tmp2/yesildau/")):
        raise ValueError(f"WP3 output is not on approved scratch: {resolved_output}")
    manifest = {
