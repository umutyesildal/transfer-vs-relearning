...
preflight_id="$(sbatch --parsable --job-name=qwen-pre-m2-3090-base-pre --output="$ROOT/logs/qwen-pre-m2-3090-base-pre-%j.out" --error="$ROOT/logs/qwen-pre-m2-3090-base-pre-%j.err" --export="ALL,EXPECTED_COMMIT=$EXPECTED_COMMIT,BASELINE_SCRATCH_ROOT=$ROOT" slurm/m2/preflight_qwen_pre_m2_baseline.slurm)"
preflight_manifest="$ROOT/preflight/manifest.json"
baseline_id="$(sbatch --parsable --dependency="afterok:$preflight_id" --job-name=qwen-pre-m2-3090-base-eval --gres=gpu:rtx3090:1 --exclude=guppi5,guppi6,guppi7 --output="$ROOT/logs/qwen-pre-m2-3090-base-eval-%A_%a.out" --error="$ROOT/logs/qwen-pre-m2-3090-base-eval-%A_%a.err" --export="ALL,EXPECTED_COMMIT=$EXPECTED_COMMIT,PREFLIGHT_MANIFEST=$preflight_manifest,BASELINE_SCRATCH_ROOT=$ROOT" slurm/m2/eval_qwen_pre_m2_baseline_slice.slurm)"
