...
preflight_id=$(sbatch --parsable slurm/m2/preflight_qwen_bridge_evaluation_recovery.slurm)
...
evaluation_id=$(sbatch --parsable --dependency="afterok:${preflight_id}" \
  ...
  slurm/m2/evaluate_qwen_bridge_recovery.slurm)
audit_id=$(sbatch --parsable --dependency="afterany:${evaluation_id}" \
  slurm/m2/audit_qwen_bridge_evaluation_recovery.slurm)
