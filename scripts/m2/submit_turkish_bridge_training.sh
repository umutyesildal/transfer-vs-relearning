...
preflight_id=$(sbatch --parsable slurm/m2/preflight_turkish_bridge_training.slurm)
...
training_id=$("${training_sbatch[@]}" slurm/m2/train_turkish_bridge.slurm)
audit_id=$(sbatch --parsable --dependency="afterany:${training_id}" \
  slurm/m2/audit_turkish_bridge_training.slurm)
