...
preflight_id=$(sbatch --parsable slurm/m2/preflight_turkish_bridge_evaluation.slurm)
...
evaluation_id=$(sbatch --parsable --dependency="afterok:${preflight_id}" \
  ...
  slurm/m2/evaluate_turkish_bridge.slurm)
audit_id=$(sbatch --parsable --dependency="afterany:${evaluation_id}" \
  slurm/m2/audit_turkish_bridge_evaluation.slurm)
