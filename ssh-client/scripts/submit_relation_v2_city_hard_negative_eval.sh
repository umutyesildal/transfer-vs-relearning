...
run_dir = repo / (
    'runs/training/m1_smollm2_360m_relation_v2_city_hard_negative_cp125/'
    '20260712T080240Z_m1_smollm2_360m_relation_v2_city_hard_negative_'
    'cp125_lr5e-6_ep1_6fd6b01c'
)
manifest = json.loads((run_dir / 'training_manifest.json').read_text(encoding='utf-8'))
if manifest.get('status') != 'complete':
    raise SystemExit(f'Canonical job 391903 run is not complete: {run_dir}')
...
if not checkpoints:
    raise SystemExit(f'No checkpoints found under canonical job 391903 run: {run_dir}')

dataset_root = repo / 'artifacts/datasets/relation_v2_gate_v1'
gate_root = dataset_root / 'acquisition_10_subjects_direct'
config_dir = repo / 'runs/local_configs/m1_relation_v2_city_hard_negative_job391903'
manifest_dir = repo / 'runs/local_model_manifests/m1_relation_v2_city_hard_negative_job391903'
...
if len(heldout_rows) != 50 or len(exact_rows) != 50:
    raise SystemExit('Expected exactly 50 held-out and 50 exact-prefix probes')
...
common = {
    'dataset_version': 'relation_v2_gate_v1',
    'dataset_dir': 'artifacts/datasets/relation_v2_gate_v1',
    'pilot_subject_file': 'artifacts/datasets/relation_v2_gate_v1/acquisition_10_subjects_direct/pilot.json',
    'languages': ['en'],
...
for checkpoint in checkpoints:
    ...
    subprocess.run(
        [
            ...
            '--model-id', f'm1_relation_v2_city_hard_negative_job391903/{checkpoint.name}',
    ...
    views = {
        'exact_prefix': {
            **common,
            ...
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_city_hard_negative_job391903_{checkpoint.name}_exact_prefix'},
        },
        'direct': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'prompt': {'format': 'direct', 'template': '{question}', 'answer_separator': ' '},
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_city_hard_negative_job391903_{checkpoint.name}_direct'},
        },
        'qa_matched': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_city_hard_negative_job391903_{checkpoint.name}_qa_matched'},
        },
    }
...
for manifest in runs/local_model_manifests/m1_relation_v2_city_hard_negative_job391903/checkpoint-*_model_manifest.json; do
