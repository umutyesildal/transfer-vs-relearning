...
run_root = repo / 'runs/training/m1_smollm2_360m_diagnostic_single_fact_answer_only'
complete_runs = []
...
if not complete_runs:
    raise SystemExit('No completed single-fact diagnostic training run found')
...
manifest_dir = repo / 'runs/local_model_manifests/m1_diagnostic_single_fact'
config_dir = repo / 'runs/local_configs/m1_diagnostic_single_fact'
...
common = {
    ...
    'pilot_subject_file': 'artifacts/datasets/acquisition_diagnostics_v1/single_fact/pilot.json',
    'languages': ['en'],
    'relations': [relation],
    'scoring': {
...
for checkpoint in checkpoints:
    ...
    subprocess.run(
        [
            ...
            '--model-id', f'm1_diagnostic_single_fact/{checkpoint.name}',
    ...
    views = {
        'exact_prefix': {
            **common,
            ...
            'probe_files': {
                'en': 'artifacts/datasets/acquisition_diagnostics_v1/single_fact/exact_prefix_probes_en.csv',
            },
            'prompt': {'format': 'direct', 'template': '{question}', 'answer_separator': ' '},
            'output': {'run_root': f'runs/evaluation/m1_diagnostic_single_fact_{checkpoint.name}_exact_prefix'},
        },
        'direct': {
            **common,
            'model_manifest': relative_manifest,
            'prompt': {'format': 'direct', 'template': '{question}', 'answer_separator': ' '},
            'output': {'run_root': f'runs/evaluation/m1_diagnostic_single_fact_{checkpoint.name}_direct'},
        },
        'qa_matched': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'output': {'run_root': f'runs/evaluation/m1_diagnostic_single_fact_{checkpoint.name}_qa_matched'},
        },
    }
...
for manifest in runs/local_model_manifests/m1_diagnostic_single_fact/checkpoint-*_model_manifest.json; do
