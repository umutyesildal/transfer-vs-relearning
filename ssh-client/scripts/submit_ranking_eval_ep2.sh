...
run_dir = repo / "runs/training/m1_smollm2_360m_english_fact_ranking/20260709T060700Z_m1_smollm2_360m_english_fact_ranking_lr1e-5_ep2_0b3462e1"
source_manifest = repo / "artifacts/models/HuggingFaceTB__SmolLM2-360M/model_manifest.json"
manifest_dir = repo / "runs/local_model_manifests/m1_smollm2_360m_english_fact_ranking_lr1e-5_ep2"
config_dir = repo / "runs/local_configs/m1_checkpoint_eval_smollm2_360m_ranking_lr1e-5_ep2"
...
for checkpoint in checkpoints:
    ...
    subprocess.run(
        [
            ...
            str(manifest_path),
            "--model-id",
            f"m1_smollm2_360m_english_fact_ranking_lr1e-5_ep2/{checkpoint}",
            "--training-checkpoint",
            checkpoint,
            "--training-run-dir",
            str(run_dir),
    ...
    direct_cfg = {
        ...
        "runtime": runtime,
        "output": {
            "run_root": f"runs/evaluation/m1_smollm2_360m_english_fact_ranking_lr1e-5_ep2_{checkpoint}_direct",
        },
    }
    qa_cfg = {
        "dataset_version": "synthetic_v1",
        ...
        "runtime": runtime,
        "output": {
            "run_root": f"runs/evaluation/m1_smollm2_360m_english_fact_ranking_lr1e-5_ep2_{checkpoint}_qa_matched",
        },
    }

    (config_dir / f"m1_smollm2_360m_english_fact_ranking_lr1e-5_ep2_{checkpoint}_direct.yaml").write_text(
        yaml.safe_dump(direct_cfg, sort_keys=False, allow_unicode=True),
        encoding="utf-8",
    )
    (config_dir / f"m1_smollm2_360m_english_fact_ranking_lr1e-5_ep2_{checkpoint}_qa_matched.yaml").write_text(
...
for cfg in runs/local_configs/m1_checkpoint_eval_smollm2_360m_ranking_lr1e-5_ep2/*.yaml; do
