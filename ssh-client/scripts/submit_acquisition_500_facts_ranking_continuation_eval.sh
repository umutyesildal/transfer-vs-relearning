...
run_root = repo / 'runs/training/m1_smollm2_360m_acquisition_500_facts_ranking_continuation'
complete_runs = []
...
if not complete_runs:
    raise SystemExit('No completed ranking-continuation run found')
...
manifest_dir = repo / 'runs/local_model_manifests/m1_acquisition_500_facts_ranking_continuation'
config_dir = repo / 'runs/local_configs/m1_acquisition_500_facts_ranking_continuation'
...
for checkpoint in checkpoints:
    ...
    subprocess.run(
        [
            ...
            '--model-id', f'm1_acquisition_500_facts_ranking_continuation/{checkpoint.name}',
    ...
    views = {
        'exact_prefix': {
            **common,
            ...
            'output': {'run_root': f'runs/evaluation/m1_acquisition_500_facts_ranking_continuation_{checkpoint.name}_exact_prefix'},
        },
        'direct': {
            **common,
            'model_manifest': relative_manifest,
            'prompt': {'format': 'direct', 'template': '{question}', 'answer_separator': ' '},
            'output': {'run_root': f'runs/evaluation/m1_acquisition_500_facts_ranking_continuation_{checkpoint.name}_direct'},
        },
        'qa_matched': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'output': {'run_root': f'runs/evaluation/m1_acquisition_500_facts_ranking_continuation_{checkpoint.name}_qa_matched'},
        },
    }
...
for checkpoint_number in ${RANKING_CONTINUATION_EVAL_CHECKPOINTS//,/ }; do
