...
run_root = repo / 'runs/training/m1_smollm2_360m_diagnostic_born_in_10_direct_answer_only'
complete_runs = []
...
if not complete_runs:
    raise SystemExit('No completed born_in 10-fact direct-supervision run found')
...
diagnostic_manifest = json.loads(
    (repo / 'artifacts/datasets/acquisition_diagnostics_v1/manifest.json').read_text(encoding='utf-8')
)
relation = diagnostic_manifest['selection']['relation']

source_manifest = repo / 'artifacts/models/HuggingFaceTB__SmolLM2-360M/model_manifest.json'
manifest_dir = repo / 'runs/local_model_manifests/m1_diagnostic_born_in_10_direct'
config_dir = repo / 'runs/local_configs/m1_diagnostic_born_in_10_direct'
...
common = {
    ...
    'pilot_subject_file': 'artifacts/datasets/acquisition_diagnostics_v1/single_relation_10_subjects_direct_supervision/pilot.json',
    'languages': ['en'],
    'relations': [relation],
    'scoring': {
...
for checkpoint in checkpoints:
    ...
    subprocess.run(
        [
            ...
            '--model-id', f'm1_diagnostic_born_in_10_direct/{checkpoint.name}',
    ...
    views = {
        'exact_prefix': {
            **common,
            ...
            'probe_files': {
                'en': 'artifacts/datasets/acquisition_diagnostics_v1/single_relation_10_subjects_direct_supervision/exact_prefix_probes_en.csv',
            },
            'prompt': {'format': 'direct', 'template': '{question}', 'answer_separator': ' '},
            'output': {'run_root': f'runs/evaluation/m1_diagnostic_born_in_10_direct_{checkpoint.name}_exact_prefix'},
        },
        'direct': {
            **common,
            'model_manifest': relative_manifest,
            'prompt': {'format': 'direct', 'template': '{question}', 'answer_separator': ' '},
            'output': {'run_root': f'runs/evaluation/m1_diagnostic_born_in_10_direct_{checkpoint.name}_direct'},
        },
        'qa_matched': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'output': {'run_root': f'runs/evaluation/m1_diagnostic_born_in_10_direct_{checkpoint.name}_qa_matched'},
        },
    }
...
for manifest in runs/local_model_manifests/m1_diagnostic_born_in_10_direct/checkpoint-*_model_manifest.json; do
