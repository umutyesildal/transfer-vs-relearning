...
if [int(path.name.split('-')[-1]) for path in checkpoints] != [25, 50, 75]:
    raise SystemExit(f'Expected checkpoints 25/50/75 under canonical job 392293 run: {run_dir}')

dataset_root = repo / 'artifacts/datasets/relation_v2_gate_v1'
gate_root = dataset_root / 'acquisition_500_subjects_direct'
config_dir = repo / 'runs/local_configs/m1_relation_v2_2500_initial_job392293'
manifest_dir = repo / 'runs/local_model_manifests/m1_relation_v2_2500_initial_job392293'
...
for checkpoint in checkpoints:
    ...
    subprocess.run(
        [
            ...
            '--model-id', f'm1_relation_v2_2500_initial_job392293/{checkpoint.name}',
    ...
    views = {
        'exact_prefix': {
            **common,
            ...
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_2500_initial_job392293_{checkpoint.name}_exact_prefix'},
        },
        'direct': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'prompt': {'format': 'direct', 'template': '{question}', 'answer_separator': ' '},
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_2500_initial_job392293_{checkpoint.name}_direct'},
        },
        'qa_matched': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_2500_initial_job392293_{checkpoint.name}_qa_matched'},
        },
    }
...
for manifest in runs/local_model_manifests/m1_relation_v2_2500_initial_job392293/checkpoint-*_model_manifest.json; do
