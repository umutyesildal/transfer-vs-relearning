...
run_dir = repo / (
    ...
    '20260711T221024Z_m1_smollm2_360m_relation_v2_10_subjects_direct_'
    'answer_only_lr1e-4_ep36_dc3dff1b'
)
manifest = json.loads((run_dir / 'training_manifest.json').read_text(encoding='utf-8'))
if manifest.get('status') != 'complete':
    raise SystemExit(f'Canonical job 391106 run is not complete: {run_dir}')
...
if not checkpoints:
    raise SystemExit(f'No checkpoints found under canonical job 391106 run: {run_dir}')

dataset_root = repo / 'artifacts/datasets/relation_v2_gate_v1'
gate_root = dataset_root / 'acquisition_10_subjects_direct'
config_dir = repo / 'runs/local_configs/m1_relation_v2_10_direct_job391106'
manifest_dir = repo / 'runs/local_model_manifests/m1_relation_v2_10_direct_job391106'
...
for checkpoint in checkpoints:
    ...
    subprocess.run(
        [
            ...
            '--model-id', f'm1_relation_v2_10_direct_job391106/{checkpoint.name}',
    ...
    views = {
        'exact_prefix': {
            **common,
            ...
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_10_direct_job391106_{checkpoint.name}_exact_prefix'},
        },
        'direct': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'prompt': {'format': 'direct', 'template': '{question}', 'answer_separator': ' '},
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_10_direct_job391106_{checkpoint.name}_direct'},
        },
        'qa_matched': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_10_direct_job391106_{checkpoint.name}_qa_matched'},
        },
    }
...
for manifest in runs/local_model_manifests/m1_relation_v2_10_direct_job391106/checkpoint-*_model_manifest.json; do
