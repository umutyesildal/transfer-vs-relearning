...
run_dir = repo / (
    'runs/training/m1_smollm2_360m_relation_v2_binding_control_10_subjects/'
    '20260712T073808Z_m1_smollm2_360m_relation_v2_binding_control_10_subjects_'
    'lr1e-4_ep36_1a765a95'
)
manifest = json.loads((run_dir / 'training_manifest.json').read_text(encoding='utf-8'))
if manifest.get('status') != 'complete':
    raise SystemExit(f'Canonical job 391889 run is not complete: {run_dir}')
...
if not checkpoints:
    raise SystemExit(f'No checkpoints found under canonical job 391889 run: {run_dir}')

dataset_root = repo / 'artifacts/datasets/relation_v2_gate_v1'
gate_root = dataset_root / 'acquisition_10_subjects_direct'
config_dir = repo / 'runs/local_configs/m1_relation_v2_binding_control_job391889'
manifest_dir = repo / 'runs/local_model_manifests/m1_relation_v2_binding_control_job391889'
...
for checkpoint in checkpoints:
    ...
    subprocess.run(
        [
            ...
            '--model-id', f'm1_relation_v2_binding_control_job391889/{checkpoint.name}',
    ...
    views = {
        'exact_prefix': {
            **common,
            ...
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_binding_control_job391889_{checkpoint.name}_exact_prefix'},
        },
        'direct': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'prompt': {'format': 'direct', 'template': '{question}', 'answer_separator': ' '},
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_binding_control_job391889_{checkpoint.name}_direct'},
        },
        'qa_matched': {
            **common,
            'model_manifest': relative_manifest,
            ...
            'output': {'run_root': f'runs/evaluation/m1_relation_v2_binding_control_job391889_{checkpoint.name}_qa_matched'},
        },
    }
...
for manifest in runs/local_model_manifests/m1_relation_v2_binding_control_job391889/checkpoint-*_model_manifest.json; do
