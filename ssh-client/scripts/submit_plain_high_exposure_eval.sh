...
run_dir = repo / "runs/training/m1_smollm2_360m_english_facts/20260708T182752Z_m1_smollm2_360m_english_facts_lr2e-5_ep5_0b566348"
source_manifest = repo / "artifacts/models/HuggingFaceTB__SmolLM2-360M/model_manifest.json"
manifest_dir = repo / "runs/local_model_manifests/m1_smollm2_360m_english_facts_lr2e-5_ep5"
config_dir = repo / "runs/local_configs/m1_checkpoint_eval_smollm2_360m_lr2e-5_ep5"
manifest_dir.mkdir(parents=True, exist_ok=True)
config_dir.mkdir(parents=True, exist_ok=True)

checkpoints = ["checkpoint-211", "checkpoint-422", "checkpoint-633", "checkpoint-844", "checkpoint-845"]
...
for checkpoint in checkpoints:
    ...
    subprocess.run(
        [
            ...
            str(manifest_path),
            "--model-id",
            f"m1_smollm2_360m_english_facts_lr2e-5_ep5/{checkpoint}",
            "--training-checkpoint",
            checkpoint,
            "--training-run-dir",
            str(run_dir),
    ...
    direct_cfg = {
        ...
        "runtime": runtime,
        "output": {
            "run_root": f"runs/evaluation/m1_smollm2_360m_english_facts_lr2e-5_ep5_{checkpoint}_direct",
        },
    }
    qa_cfg = {
        "dataset_version": "synthetic_v1",
        ...
        "runtime": runtime,
        "output": {
            "run_root": f"runs/evaluation/m1_smollm2_360m_english_facts_lr2e-5_ep5_{checkpoint}_qa_matched",
        },
    }

    (config_dir / f"m1_smollm2_360m_english_facts_lr2e-5_ep5_{checkpoint}_direct.yaml").write_text(
        yaml.safe_dump(direct_cfg, sort_keys=False, allow_unicode=True),
        encoding="utf-8",
    )
    (config_dir / f"m1_smollm2_360m_english_facts_lr2e-5_ep5_{checkpoint}_qa_matched.yaml").write_text(
