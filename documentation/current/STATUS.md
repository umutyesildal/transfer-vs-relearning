...
**As of:** 2026-08-22 | **Phase:** Pile-10k retired; eval-v2 frozen; M0 projection, source audit and canonical normalization complete; M1 evidence blockers under read-only resolution
...
The local read-only audit confirms eval-v2 protocol identity but finds the Turkish benchmark
identity reconciliation, model artifact provenance, measurement review fields and primary
in-domain corpus split still unresolved.
...
the current draft reports the missing synthetic-fact/checkpoint/training manifests, unresolved
exact-prefix registry hash, execution authorization and adapter. vngrs is deliberately absent from
this M1 input packet because it belongs to the later matched M2-A/M2-B Turkish sibling contract.
The command does not load a model, call LM Eval, touch HU, submit Slurm or create an evaluation root.
Once a separately hash-bound M1 contract supplies the required identities and registers adapters,
...
The completed bounded tasks were the read-only eval-v2 M0 metric-schema audit v1c and the v1f
...
4. the audit passed with 24 source rows and 42 observations, and v1f wrote the canonical 42-row metric table;
